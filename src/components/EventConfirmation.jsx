    ...
    try {
      ...
      if (err.response && err.response.status) {
        if (err.response.status === 401) {
          setSubmitError(
            'Error de autenticación. Contacta al administrador de Baraki.'
          );
        } else if (err.response.status === 409) {
          setSubmitError(
            'Ya existe una confirmación de asistencia con este email. ¡Gracias por confirmar!'
          );
        } else if (err.response.status === 400 && err.response.data.message) {
          setSubmitError(err.response.data.message);
        } else {
          setSubmitError(
            'No se pudo enviar la confirmación. Error en el servidor o red. Por favor, intenta de nuevo.'
          );
        }
      } else {
        setSubmitError(
  ...
  return (
    <div ref={formRefProp} className="mt-16 card-elegant slide-up">
      ...
      <form
        ...
        noValidate
        className={`
          max-w-2xl mx-auto transition-all duration-700 ease-in-out overflow-hidden
          ${
            formSubmitted
      ...
      >
        ...
        <div className="bg-green-50 border border-green-200 rounded-xl p-6 mb-6">
          ...
          <p className="text-green-700 text-sm">
            Los detalles del evento han sido guardados.
          </p>
        </div>
        <button
          onClick={() => {
